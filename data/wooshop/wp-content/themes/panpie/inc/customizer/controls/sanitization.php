...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    if ( ! function_exists( 'rttheme_is_topbar1_enabled' ) ) {
        function rttheme_is_topbar1_enabled() {
            $top_bar_style = get_theme_mod( 'top_bar_style', '1' );
            if ( $top_bar_style == 1 ) {
                return true;
            }
            return false;
    ...
    if ( ! function_exists( 'rttheme_is_topbar2_enabled' ) ) {
        function rttheme_is_topbar2_enabled() {
            $top_bar_style = get_theme_mod( 'top_bar_style', '2' );
            if ( $top_bar_style == 2 ) {
                return true;
            }
            return false;
    ...
    if (!function_exists('rttheme_is_related_gallery_enabled')) {
        function rttheme_is_related_gallery_enabled()
        {
            $show_related_port = get_theme_mod('show_related_port');
            if (empty($show_related_port)) {
                return false;
            }
            return true;
        }
	...
	/* Event related option */
    if (!function_exists('rttheme_is_related_event_enabled')) {
        function rttheme_is_related_event_enabled()
        {
            $show_related_event = get_theme_mod('show_related_event');
            if (empty($show_related_event)) {
                return false;
            }
            return true;
        }
     ...
     * Check is selected banner background type is image
     */
    if ( ! function_exists( 'rttheme_single_gallery_bgimg_type_condition' ) ) {
        function rttheme_single_gallery_bgimg_type_condition() {
            $select_value = get_theme_mod( 'single_gallery_banner', false );
            $SgBgType = get_theme_mod( 'single_gallery_banner_bg_type', 'bgimg' );
     ...
     * Check is selected banner background type is color
     */
    if ( ! function_exists( 'rttheme_single_gallery_bgcolor_type_condition' ) ) {
        function rttheme_single_gallery_bgcolor_type_condition() {
            $select_value = get_theme_mod( 'single_gallery_banner', false );
            $select_bg_value = get_theme_mod( 'single_gallery_banner_bg_type', 'bgcolor' );
            if ( !empty( $select_value ) && $select_bg_value == 'bgcolor' ) {
                return true;
            }
            return false;
	...
	if ( ! function_exists( 'rttheme_footer_bgimg_type_condition' ) ) {
        function rttheme_footer_bgimg_type_condition() {
	...
	if ( ! function_exists( 'rttheme_footer_bgcolor_type_condition' ) ) {
        function rttheme_footer_bgcolor_type_condition() {
            $BgType = get_theme_mod('footer_bgtype');
            if ( $BgType === 'fbgcolor' ) {
                return true;
            }
