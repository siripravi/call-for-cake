			...
			switch ( $schemaCat ) {
				...
				case 'course':
					...
					if ( ! empty( $metaData['duration'] ) ) {
						$courseSchedule['duration'] = $helper->sanitizeOutPut( $metaData['duration'] );
					}
					if ( ! empty( $metaData['repeatFrequency'] ) ) {
						$courseSchedule['repeatFrequency'] = $helper->sanitizeOutPut( $metaData['repeatFrequency'] );
					}
					if ( ! empty( $metaData['repeatCount'] ) ) {
						$courseSchedule['repeatCount'] = $helper->sanitizeOutPut( $metaData['repeatCount'] );
					}
					...
					if ( ! empty( $metaData['price'] ) && ! empty( $metaData['priceCurrency'] ) ) {
						...
						if ( ! empty( $metaData['category'] ) ) {
							$course['offers']['category'] = $helper->sanitizeOutPut( $metaData['category'] );
						}
						if ( ! empty( $metaData['availability'] ) ) {
							$course['offers']['availability'] = $helper->sanitizeOutPut( $metaData['availability'] );
						}
						if ( ! empty( $metaData['url'] ) ) {
							$course['offers']['url'] = $helper->sanitizeOutPut( $metaData['url'] );
						}
						if ( ! empty( $metaData['validFrom'] ) ) {
							$course['offers']['validFrom'] = $helper->sanitizeOutPut( $metaData['validFrom'] );
						}
					}
				...
				case 'profile_page':
					...
					if ( ! empty( $metaData['sameAs'] ) ) {
						$mainEntity['sameAs'] = Functions::get_same_as( $helper->sanitizeOutPut( $metaData['sameAs'], 'textarea' ) );
					}
					...
					if ( ! empty( $metaData['worksFor'] ) ) {
						...
						foreach ( $metaData['worksFor'] as $details ) {
							...
							if ( ! empty( $details['logo'] ) ) {
								...
								$for['logo'] = $helper->sanitizeOutPut( $img['url'], 'url' );
							}
							if ( ! empty( $details['sameAs'] ) ) {
								$for['sameAs'] = Functions::get_same_as( $helper->sanitizeOutPut( $details['sameAs'], 'textarea' ) );
							}
							$department = [];
					...
					// End Main $mainEntity.
					if ( ! empty( $metaData['dateCreated'] ) ) {
						$profile_page['dateCreated'] = $helper->sanitizeOutPut( $metaData['dateCreated'] );
					}
					if ( ! empty( $metaData['dateModified'] ) ) {
						$profile_page['dateModified'] = $helper->sanitizeOutPut( $metaData['dateModified'] );
					}
					if ( ! empty( $mainEntity ) ) {
					...
					if ( $without_script ) {
						$html = apply_filters( 'rtseo_snippet_profile_page', $profile_page, $metaData );
					} else {
						$html .= $this->getJsonEncode( apply_filters( 'rtseo_snippet_profile_page', $profile_page, $metaData ) );
					}
