<?php
	...
	foreach ( $settings['slider_items'] as $item ) {
		...
		echo '<h2 class="staggered-animation" data-animation="slideInLeft" data-animation-delay="0.5s">'.esc_html($item['slider_title']).'</h2>';
		echo '<h5 class="mb-3 mb-sm-4 staggered-animation font-weight-light" data-animation="slideInLeft" data-animation-delay="1s">'.shopwise_sanitize_data($item['slider_subtitle']).'</h5>';
		if($item['slider_btn_title']){
		echo '<a class="btn btn-fill-out staggered-animation text-uppercase" href="'.esc_url($item['slider_btn_link']['url']).'" '.esc_attr($target.$nofollow).' data-animation="slideInLeft" data-animation-delay="1.5s">'.esc_html($item['slider_btn_title']).'</a>';
		}
		echo '</div>';
		echo '</div>';
		echo '</div>';
