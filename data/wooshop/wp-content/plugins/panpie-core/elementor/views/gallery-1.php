...
<div class="gallery-default gallery-multi-layout-1 gallery-<?php echo esc_attr( $data['layout'] );?>">
	<div class="row rt-masonry-grid <?php echo esc_attr( $gap_class ); ?>">	
		<?php $i = 1;
			if ( $query->have_posts() ) {
				...
				$panpie_port_no  = get_post_meta( $id, 'panpie_port_no', true );				
				$thumb_size			= 'panpie-size' . rand( 3, 5 ) ;
		?>
		<div class="rt-grid-item <?php if ( ( $i == $posts_per_page ) && ( $posts_per_page % 2 == 1 ) ) { echo esc_attr( $is_offset ) ; } echo esc_attr( $col_class ) ?>">
			<div class="rtin-item">
				...
				<div class="rtin-content">
					...
					<?php } ?>
					<h3><a href="<?php echo wp_get_attachment_url( get_post_thumbnail_id() ); ?>" class="img-popup-icon" data-elementor-open-lightbox="yes" data-elementor-lightbox-slideshow="1" data-elementor-lightbox-title="<?php echo get_the_title(); ?>"><?php the_title();?></a></h3>
					<?php if ( $data['excerpt_display'] == 'yes' ) { ?>
					<p><?php echo wp_kses( $excerpt , 'alltext_allow' ); ?></p>
					<?php } ?>
					<?php if ($data['view_display'] == 'yes' || $data['link_display'] == 'yes' ) { ?>
					<div class="item-icon">
