...
if( !empty( $data['price'] )){
	$price_html  = '<span class="price-symbol">' . $data['price_symbol'] . '</span>' . $data['price'] . '<span class="price-unit">'. $data['faction'] . '</span>';
} else {
	$price_html = '';
}

if ( !empty( $data['buttontext'] ) ) {
	$btn = '<a class="panpie-button-1" ' . $attr . '>' . $data['buttontext'] . '<i class="fas fa-arrow-right"></i>' . '</a>';
}
...
<div class="info-box info-<?php echo esc_attr( $data['style'] );?>">
	<div class="rtin-item media-<?php echo esc_attr( $data['icontype'] );?>">
		<div class="rtin-content">
			...
			<?php } if ( !empty( $data['start'] ) ) { ?>
			<div class="rtin-start"><?php echo wp_kses_post( $data['start'] );?></div>
			<?php } ?>
