...
* By : Radius Theme
*/
class PanpieTheme_Open_Hour_Widget extends WP_Widget {
	public $weekdays;
	public function __construct() {
		$widget_ops = array(
			'classname' => 'widget-open-hour',
			'description' => esc_html__( 'Display the opening hours' , 'panpie-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'open-hour', esc_html__( 'Panpie : Opening Hours' , 'panpie-core' ), $widget_ops );	
		
		$this->weekdays = array(
			'monday'   => esc_html__( 'Monday', 'panpie-core' ),
			'tuesday'  => esc_html__( 'Tuesday', 'panpie-core' ),
			'wednesday'=> esc_html__( 'Wednesday', 'panpie-core' ),
			'thursday' => esc_html__( 'Thursday', 'panpie-core' ),
			'friday'   => esc_html__( 'Friday', 'panpie-core' ),
			'saturday' => esc_html__( 'Saturday', 'panpie-core' ),
			'sunday'   => esc_html__( 'Sunday', 'panpie-core' ),
		);
	}
		...
		<ul class="opening-schedule">
			...
			<?php if( !empty( $from_label ) || !empty( $to_label ) ){ ?>
					<li><?php echo esc_html( $weekday ); ?><?php if( !empty( $from_label ) && !empty( $to_label ) ){ ?><span>: <?php echo esc_html( $from_label ); ?> - <?php echo esc_html( $to_label ); ?> </span>
						<?php } else { ?><span class="os-close">: <?php esc_html_e('Closed' , 'panpie-core');?></span><?php } ?>
					</li>
				<?php } ?>
			<?php } ?>
		</ul>
	...
	public function form( $instance ) {		
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'panpie-core' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<table>
			<tr>
				<th style="width: 30%"><?php esc_html_e( 'Days' , 'panpie-core'); ?></th>				
				<th style="width: 30%"><?php esc_html_e( 'From' , 'panpie-core'); ?></th>				
				<th style="width: 10%; text-align: center"></th>
				<th style="width: 30%"><?php esc_html_e( 'To' , 'panpie-core'); ?></th>
			</tr>
		<?php foreach ( $this->weekdays as $key=>$weekday ) { ?>
			<?php			
				$from_label = $key.'_from';
